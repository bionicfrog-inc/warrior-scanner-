...
function renderRanking(){
  ...
  el.innerHTML=valid.map((r,i)=>{
    const s=+r.score||0,col=sc(s),chg=+r.variation||0,rvol=+r.rvol||0,flRaw=r.float_m,hasFloat=flRaw!==undefined&&flRaw!==null&&flRaw!=="",fl=hasFloat?(+flRaw||0):null,flText=r.float||(hasFloat?n(fl,1)+"M":"N/A");
    ...
    if(hasFloat&&fl<=5) badges+='<span class="badge badge-amber">🎯 Low Float</span>';
    ...
    <div class="rank-row">
      <div class="rank-main">
        ...
        <div style="color:${hasFloat?(fl<=10?'#4ade80':fl<=20?'#fbbf24':'#a0aec0'):'#a0aec0'};font-family:IBM Plex Mono,monospace;font-size:13px">${flText}</div>
        <div>
...
function renderIndicators(){
  const cols=['symbol','prix','variation','gap %','volume','rvol','avg vol 10j','float','float m','sma50','sma200','52w high','dist 52w %'];
  const labels={'symbol':'Symbole','prix':'Prix','variation':'Var%','gap %':'Gap%','volume':'Volume','rvol':'RVOL','avg vol 10j':'AvgVol10j','float':'Float','float m':'Float M','sma50':'SMA50','sma200':'SMA200','52w high':'52W High','dist 52w %':'Dist 52W%'};
  ...
  document.getElementById('ind-body').innerHTML=valid.map(r=>`<tr>${avail.map(c=>{
    ...
    if(c==='float')return`<td>${v||'N/A'}</td>`;
    if(c==='float m'){if(v===''||v===undefined)return`<td>N/A</td>`;const fm=+v||0;return`<td style="color:${fm<=10?'#4ade80':fm<=20?'#fbbf24':'#a0aec0'}">${n(fm,1)}M</td>`;}
...
RENAME = {
    ...
    "Float": "float", "Float M": "float_m", "Market Cap": "market_cap",
...
def load_data():
    ...
    with open(CSV_PATH, newline="", encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            ...
            if not renamed.get("float"):
                fm = renamed.get("float_m", "")
                try:
                    renamed["float"] = f"{float(fm):.1f}M" if fm else "N/A"
                except ValueError:
                    renamed["float"] = "N/A"
